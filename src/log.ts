...
export class LogEntry {
	...
	static fromInput(input: LogInput, now: Date): LogEntry {
		...
		return new LogEntry(input.message, place, applyTime(now, input.from), now);
	}
...
export class WeeklyLog {
	...
	private findSection(header: string): number | null {
		const idx = this.lines.findIndex((line) => line.trim() === header);
	...
	private findFirstDateHeader(): number | null {
		for (let i = 0; i < this.lines.length; i++) {
			...
			if (stripped.startsWith('# 20') && stripped.length === 12) {
				return i;
			}
		}
		return null;
